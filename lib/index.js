...
const execa = require("spawno").promise
...
module.exports = class BloggifyTools {
    ...
    static exec (cmd, args, cwd, stdin) {
        const proc = spawno(cmd, args, { cwd, stdio: 'inherit'})
    ...
    static databaseConnect () {
        ...
        switch (protocol) {
            ...
            case "mysql":
                ...
                return BloggifyTools.exec("mariadb", args, appRoot, true)

            case "sqlite":
                return BloggifyTools.exec("sqlite3", [dbUrl.replace("sqlite://", "")])
    ...
    static runMigrations () {
        ...
        if (env.DB_URI.startsWith("sqlite")) {
            const configPath = appRoot + "/config/config.json"
            if (!isThere(configPath)) {
                writeFile.sync(configPath, JSON.stringify({
                    development: {
                        dialect: "sqlite",
                        storage: env.DB_URI.replace("sqlite://", "")
                    }
                }, null, 4))
            }
            return BloggifyTools.exec("npx", ["sequelize-cli", "db:migrate"], appRoot)
        }
