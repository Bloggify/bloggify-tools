...
BloggifyTools.prototype.init = function (options, callback) {
    ...
    Ul.merge(options, { name: "Bloggify", conf: {} });
    ...
    r.exec(OArgv({
        ...
        ev.emit("progress", { type: "info", content: "Installing dependencies." });
        this.npmInstall(function (err) {
            ...
            this.initConfig(options, function (err) {
                if (err) { return callback(err); }
 ...
 * @return {undefined}
 */
BloggifyTools.prototype.initConfig = function (options, callback) {
    FsExtra.writeJSON(this.path + "/conf/index.json", options.conf, callback);
};
